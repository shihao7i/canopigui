 ...
 * 
   ...
   <div tree-table column-definition="columnDefinition"></div>
...
angular.module('maxmedia.directive').directive('treeTable', ['$timeout', '$log', '$document', '$compile', function ($timeout, $log, $document, $compile) {
	'use strict';
	...
	return {
	    ...
	    template: '<div class="table-responsive" id="treeTable">' +
                  ...
                  '          <tr>' +
                  '              <th class="tree-table-nav-reset" ng-click="treeTableNavResetClicked($event)">' +
                  '                   <span class="icon-ICON_EXPAND_ALL_wht"></span>' +
                  '              </th>' +
                  '              <th ng-repeat="column in columnDefinition.tableRows.rowMetaData.columnList ">{{column.displayName}}</th>' +
                  '          </tr>' +
                  '      </thead>' +
                  '      <tbody ng-repeat="row in columnDefinition.tableRows.rowValueList track by row.id">' +
                  '          <tr>' +
                  '              <td id="tree-table-content-{{row.id}}" class="tree-table-nav" ng-click="treeTableNavClicked($event)">' +
		...
		link: function ( scope, element, attrs ) {
			...
			scope.treeTableNavClicked = function(e) {
	
				var elem = angular.element(e.currentTarget);
		
				var id = elem.attr('id');
		       // var reset = angular.element(document.querySelector('.tree-table-nav-reset'));
		       // var classId = angular.element(document.querySelector('.'+id));
			...
			scope.treeTableNavResetClicked = function(e) {
	
				var elem = angular.element(e.currentTarget);
			
				var ourTableId = '#' + elem.parent().parent().parent().parent().attr('id');
				
				//var treeTableContent = angular.element(document.querySelector(ourTableId + ' .tree-table-content'));
				//var treeTableNav = angular.element(document.querySelector(ourTableId + ' .tree-table-nav'));
