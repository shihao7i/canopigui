angular.module('canopi.app').controller('DeviceDetailsController', ['$scope', '$location', '$rootScope', '$state', 'Cache', '$filter', '$http',
    function ($scope, $location, $rootScope, $state, Cache, $filter, $http) {
        ...
        function setupScopeValuesAndMethods() {
            ...
            $http.get('app/canopi-app/mock/telco/network/ipAddressesResponse.json')
                .then(function (res) {
                    vm.ipAddressesResponseData = res.data;
                    processResponseResult(vm.ipAddressesData, vm.ipAddressesResponseData);
                });

            $http.get('app/canopi-app/mock/telco/network/cardsOrPortsResponse.json')
                .then(function (res) {
                    vm.cardsOrPortsResponseData = res.data;
                    processResponseResult(vm.cardsOrPortsData, vm.cardsOrPortsResponseData);
                });
