// angular-formly-templates-bootstrap version 6.1.0 built with â™¥ by Astrism <astrisms@gmail.com>, Kent C. Dodds <kent@doddsfamily.us> (Ã³ Ã¬_Ã­)=Ã³Ã²=(Ã¬_Ã­ Ã²)
...
(function webpackUniversalModuleDefinition(root, factory) {
  if (typeof exports === 'object' && typeof module === 'object')
    ...
    exports['ngFormlyTemplatesBootstrap'] = factory(
      require('angular-formly'),
      require('api-check'),
      require('angular')
    );
  else
    root['ngFormlyTemplatesBootstrap'] = factory(
      root['ngFormly'],
      root['apiCheck'],
      root['angular']
    );
            ...
            formlyConfigProvider.setType({
              ...
              controller: /* @ngInject */ [
                ...
                function controller($scope) {
                  ...
                  $scope.$watch(
                    'model',
                    function modelWatcher(newModelValue) {
                      var modelValue, valueProp;

                      if (Object.keys(newModelValue).length) {
                        modelValue = newModelValue[opts.key];

                        $scope.$watch('to.options', function optionsWatcher(
                          newOptionsValues
                        ) {
                          if (
                            newOptionsValues &&
                            Array.isArray(newOptionsValues) &&
                            Array.isArray(modelValue)
                          ) {
                            valueProp = to.valueProp || 'value';
                            for (
                              var index = 0;
                              index < newOptionsValues.length;
                              index++
                            ) {
                              $scope.multiCheckbox.checked[index] =
                                modelValue.indexOf(
                                  newOptionsValues[index][valueProp]
                                ) !== -1;
                            }
                          }
                        });
                  ...
                  function checkValidity(expressionValue) {
                    ...
                    if ($scope.to.required) {
                      valid =
                        angular.isArray($scope.model[opts.key]) &&
                  ...
                  function setModel() {
                    ...
                    angular.forEach($scope.multiCheckbox.checked, function(
                      checkbox,
                      index
                    ) {
                      if (checkbox) {
                        $scope.model[opts.key].push(
                          to.options[index][to.valueProp || 'value']
                        );
                      }
                    });
                  ...
                  if (
                    ...
                    opts.expressionProperties['templateOptions.required']
                  ) {
                    $scope.$watch(
                      function() {
                  ...
                  if ($scope.to.required) {
                    var unwatchFormControl = $scope.$watch('fc', function(
                      newValue
                    ) {
                      if (!newValue) {
                        return;
      ...
      /***/ function(module, exports, __webpack_require__) {
        ...
        exports['default'] = function(ngModule) {
          ...
          function addAddonsManipulator(formlyConfig, formlyBootstrapApiCheck) {
            ...
            formlyConfig.templateManipulators.preWrapper.push(function(
              template,
              options
            ) {
              if (
                !options.templateOptions.addonLeft &&
                !options.templateOptions.addonRight
              ) {
                return template;
              }
        ...
        exports['default'] = function(ngModule) {
          ...
          function addDescriptionManipulator(formlyConfig) {
            formlyConfig.templateManipulators.preWrapper.push(
              function ariaDescribedBy(template, options, scope) {
                if (angular.isDefined(options.templateOptions.description)) {
                  ...
                  var modelEls = angular.element(
                    el.querySelectorAll('[ng-model]')
                  );
                  if (modelEls) {
                    modelEls.attr(
                      'aria-describedby',
                      scope.id + '_description'
                    );
                  }
                  return el.innerHTML;
                } else {
