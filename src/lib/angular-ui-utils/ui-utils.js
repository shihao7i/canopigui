        ...
        compile: function uiMaskCompilingFunction() {
          ...
          return function uiMaskLinkingFunction(
            ...
            function eventHandler(e) {
              ...
              var val = iElement.val(),
                ...
                isDeletion =
                  ...
                  (selectionLenOld &&
                    val.length === valOld.length - selectionLenOld),
                isSelection =
                  eventWhich >= 37 && eventWhich <= 40 && e.shiftKey, // Arrow key codes
              ...
              if (
                isSelection ||
                (isSelected && (eventType === 'click' || eventType === 'keyup'))
              ) {
                return;
              }
    ...
    return {
      ...
      compile: function(tElement, tAttrs) {
        ...
        if (tAttrs.uiRoute) {
          ...
          throw new Error(
            'uiRoute missing a route or href property on ' + tElement[0]
          );
        }
        return function($scope, elm, attrs) {
          var modelSetter = $parse(
            attrs.ngModel || attrs.routeModel || '$uiRoute'
          ).assign;
      ...
      return {
        registerFor: function(element) {
          ...
          getMeasurements = function(elem, measure) {
            ...
            borderA =
              convertToPx(elem, computedStyle['border' + dirA + 'Width']) || 0;
            borderB =
              convertToPx(elem, computedStyle['border' + dirB + 'Width']) || 0;
        ...
        compile: function(element, attr, linker) {
          return function($scope, $element, $attr, controllers) {
            ...
            shouldLoadBottom = function() {
              return (
                !eof &&
                adapter.bottomDataPos() < bottomVisiblePos() + bufferPadding()
              );
            };
            clipBottom = function() {
              ...
              overage = 0;
              for (
                i = _i = _ref = buffer.length - 1;
                _ref <= 0 ? _i <= 0 : _i >= 0;
                i = _ref <= 0 ? ++_i : --_i
              ) {
                itemHeight = buffer[i].element.outerHeight(true);
                if (
                  adapter.bottomDataPos() - bottomHeight - itemHeight >
            ...
            clipTop = function() {
              ...
              for (_i = 0, _len = buffer.length; _i < _len; _i++) {
                ...
                if (
                  adapter.topDataPos() + topHeight + itemHeight <
            ...
            adjustRowHeight = function(appended, wrapper) {
              var newHeight;
              if (appended) {
                return adapter.bottomPadding(
                  Math.max(
                    0,
                    adapter.bottomPadding() - wrapper.element.outerHeight(true)
                  )
                );
              } else {
                newHeight =
                  adapter.topPadding() - wrapper.element.outerHeight(true);
                if (newHeight >= 0) {
                  return adapter.topPadding(newHeight);
                } else {
                  return viewport.scrollTop(
                    viewport.scrollTop() + wrapper.element.outerHeight(true)
                  );
                }
              }
            };
            ...
            return eventListener.$on('insert.item', function(
              event,
              locator,
              item
            ) {
      ...
      return {
        ...
        link: function(scope, elm, attrs, uiScrollfixTarget) {
          var top = elm[0].offsetTop,
            $target =
              (uiScrollfixTarget && uiScrollfixTarget.$element) ||
          ...
          function onScroll() {
            ...
            if (!elm.hasClass('ui-scrollfix') && offset > attrs.uiScrollfix) {
              ...
              elm.hasClass('ui-scrollfix') &&
              offset < attrs.uiScrollfix
            ) {
