    ...
    AtmosphereRequest: function(options) {
          ...
          connectors = {
            storage: function() {
              ...
              var storage = window.localStorage,
                get = function(key) {
                  return atmosphere.util.parseJSON(
                    storage.getItem(name + '-' + key)
                  );
                },
                set = function(key, value) {
                  storage.setItem(
                    name + '-' + key,
                    atmosphere.util.stringifyJSON(value)
                  );
                };
      ...
      function _buildWebSocketUrl() {
        return _attachHeaders(
          _request,
          atmosphere.util.getAbsoluteURL(_request.url)
        ).replace(/^http/, 'ws');
      }
      ...
      function _executeRequest(request) {
        ...
        if (
          ...
          try {
            ...
            atmosphere.util.log(rq.logLevel, [
              'Unable to connect to ' + rq.url
            ]);
            _onError(0, e);
          }
        } else {
