angular.module('canopi.app').controller('EquipmentSearchController', [
  '$scope',
  '$location',
  '$rootScope',
  '$state',
  'InventoryJsonService',
  'Cache',
  '$filter',
  function(
    $scope,
    ...
    function populatePicklistValues() {
      ...
      $rootScope.$watch('picklists', function() {
        ...
        if ($rootScope.picklists !== undefined) {
          ...
          $scope.deviceTypeAndSubtypesList =
            $rootScope.picklists.deviceTypeAndSubtypesList;
          var tempDeviceTypeList = new Array();

          angular.forEach($scope.deviceTypeAndSubtypesList, function(value) {
            angular.forEach(value.deviceTypeSubTypesMap, function(
              value2,
              key2
            ) {
          ...
          angular.forEach($rootScope.picklists.metaStatusesList, function(
            value
          ) {
    ...
    $scope.showHideUVerseBanField = function() {
      ...
      angular.forEach($rootScope.picklists.uverseBanDeviceSubtypeList, function(
        value
      ) {
    ...
    function validateFormFields() {
      ...
      angular.forEach(
        $rootScope.picklists.locationClliValidationDeviceList,
        function(value) {
          if (
            $scope.equipmentSearch.deviceType != '' &&
            value.indexOf($scope.equipmentSearch.deviceType) >= 0
          ) {
