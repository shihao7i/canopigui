angular.module('canopi.app').controller('NetworkDiscoveryController', [
  '$scope',
  '$location',
  '$rootScope',
  '$state',
  ...
  function($scope, $location, $rootScope, $state, Cache, $filter, $http) {
    ...
    function setupScopeValuesAndMethods() {
      ...
      $http
        .get('app/mock/telco/network/networkDetailsResponse.json')
        .then(function(res) {
          $scope.networkDetailsResponseData = res.data;
          processResponseResult(
            $scope.networkDetailsData,
            $scope.networkDetailsResponseData
          );
        });

      $http
        .get('app/mock/telco/network/deviceClliInfoResponse.json')
        .then(function(res) {
          $scope.deviceClliInfoResponseData = res.data;
          processResponseResult(
            $scope.deviceClliInfo,
            $scope.deviceClliInfoResponseData
          );
        });
    }
