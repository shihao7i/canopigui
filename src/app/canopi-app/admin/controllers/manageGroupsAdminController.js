angular.module('canopi.app').controller('ManageGroupsAdminController', [
  '$scope',
  '$log',
  '$location',
  '$rootScope',
  ...
  function(
    ...
    function setupScopeVariablesAndMethods() {
      ...
      var templateWithTooltip =
        '<div tooltip="{{row.entity.description}}" tooltip-animation="false" tooltip-append-to-body="true" tooltip-placement="left" >{{row.getProperty(col.field)}}</div>';
      ...
      $scope.groupAffiliateOptions = {
        ...
        afterSelectionChange: function(row, event) {
          ...
          if (
            $scope.groupAffiliateSelectedItems !== null ||
            $scope.groupAffiliateSelectedItems !== undefined
          ) {
      ...
      $scope.deleteWorkGroup = function() {
        ...
        $scope.getAllWorkGroups.workGrouplist.splice(
          $scope.getAllWorkGroups.workGrouplist.indexOf(
            $scope.groupAffiliateSelectedItems[0]
          ),
          1
        );
        promise.then(function(data) {
      ...
      $scope.removeUserInWorkGroup = function() {
        ...
        $scope.usersInWorkGroup.userlist.splice(
          $scope.usersInWorkGroup.userlist.indexOf(
            $scope.getUserInWorkGroupSelectedItems[0]
          ),
          1
        );
      ...
      $scope.removePermissionFromGroup = function(itemsToBeRemoved) {
        var postObject = {
          workGroup: {
            ...
            permissionDTOs:
              $scope.groupAffiliateSelectedItems[0].permissionDTOs,
        ...
        promise.then(function(data) {
          $log.debug(
            'removePermissionFromGroup => ' + JSON.stringify(data, null, '\t')
          );
        });
        return promise;
      };
      ...
      $scope.addPermissionToGroup = function(itemsToBeAdded) {
        ...
        promise.then(function(data) {
          $log.debug(
            'addPermissionToGroup => ' + JSON.stringify(data, null, '\t')
          );
        });
        return promise;
      };
      ...
      $scope.addAdminToGroup = function() {
        if (
          $scope.listAllUsersInfo !== undefined ||
          $scope.listAllUsersInfo !== null
        ) {
          if ($scope.usingAddGroupModal === true) {
            ...
            $scope.addAdminFilter.filterText =
              $scope.originalAdminFilter.filterText;
          } else {
            addAdminToWorkGroup(
              $scope.groupAffiliateOptions.selectedItems[0].administrators,
              $scope.addAdminSelectedItems
            );
            $scope.addAdminFilter.filterText =
              $scope.originalAdminFilter.filterText;
      ...
      $scope.clearAddGroupModal = function() {
        ...
        $scope.saveNewWorkGroup.affiliate =
          $scope.saveNewWorkGroupAffiliateList[0];
        $scope.usingAddGroupModal = false;
      };
      ...
      $scope.clearModifiedGroupModal = function() {
        ...
        clearSelectedItems($scope.addAdminSelectedItems);
        $scope.groupAffiliateOptions.selectedItems[0].name =
          $scope.originalAdminData.name;
        $scope.groupAffiliateOptions.selectedItems[0].description =
          $scope.originalAdminData.description;
        $scope.groupAffiliateOptions.selectedItems[0].administrators =
          $scope.originalAdminData.administrators;
      };
      ...
      $scope.setupModifyModal = function() {
        ...
        angular.copy(
          $scope.groupAffiliateOptions.selectedItems[0],
          $scope.originalAdminData
        );
      };
        ...
        promise.then(function(data) {
          ...
          if ($scope.groupAffiliateSelectedItems[0].permissions !== null) {
            if ($scope.groupAffiliateSelectedItems[0].affiliate === 'TELCO') {
              ...
              $scope.groupAffiliateSelectedItems[0].affiliate === 'MOBILITY'
            ) {
      ...
      function filterPermissionsListData(ngGridData) {
        ...
        if (
          $scope.getGroupPermissionsList.permissionlist !== null ||
          $scope.getGroupPermissionsList.permissionlist !== undefined
        ) {
          for (
            i = 0;
            i < $scope.getGroupPermissionsList.permissionlist.length;
            i++
          ) {
            for (j = 0; j < ngGridData.length; j++) {
              if (
                $scope.getGroupPermissionsList.permissionlist[i].name ===
                ngGridData[j].name
              ) {
      ...
      function populatePicklistValues() {
        $scope.saveNewWorkGroupAffiliateList = [
          { name: '' },
          { name: 'Telco' },
          { name: 'Mobilty' },
          { name: 'Admin' }
