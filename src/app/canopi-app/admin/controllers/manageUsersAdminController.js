angular.module('canopi.app').controller('ManageUsersAdminController', [
  '$scope',
  '$log',
  '$location',
  '$rootScope',
  ...
  function(
    ...
    function setupScopeValuesAndMethods() {
      ...
      var templateWithTooltip =
        '<div class="ngCellText"><a tooltip="{{row.entity.description}}" tooltip-append-to-body="true" tooltip-placement="left" ng-cell-text>{{row.getProperty(col.field)}}</a></div>';
    ...
    $scope.userAdd = function() {
      ...
      for (var i = 0; i < userIdData.length; i++) {
        if (
          userIdData[i].attUid.toLowerCase() ==
