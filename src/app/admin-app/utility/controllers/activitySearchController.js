(function() {
  ...
  activitySearchController.$inject = [
    '$scope',
    '$log',
    '$filter',
    'picklists',
  ...
  function activitySearchController(
    $scope,
    $log,
    $filter,
    picklists,
    ...
    function activitySearch() {
      ...
      AdminJsonService.getActivitySearchResults().then(function(data) {
        ...
        vm.gridOptionsActivityResultsTable.enableHorizontalScrollbar = UiGridUtilService.isHorizontalScrollbarNeeded(
          colDefs
        );
        vm.gridOptionsActivityResultsTable.columnDefs = colDefs;
        vm.gridOptionsActivityResultsTable.exporterCsvFilename =
          'activities.csv';

        //HIDE ID COLUMN
        for (
          var i = 0;
          i < vm.gridOptionsActivityResultsTable.columnDefs.length;
          i++
        ) {
          if (vm.gridOptionsActivityResultsTable.columnDefs[i].id === 'id') {
            vm.gridOptionsActivityResultsTable.columnDefs[
              i
            ].enableHiding = false;
            break;
          }
    ...
    function refreshData(filter) {
      ...
      while (filter) {
        ...
        vm.gridOptionsActivityResultsTable.data = $filter('filter')(
          vm.gridOptionsActivityResultsTable.data,
          oSearchArray[0],
          undefined
        );
        oSearchArray.shift();
