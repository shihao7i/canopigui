(function() {
    ...
    lookupAdminController.$inject = ['$scope', '$log', '$filter', '$interval', 'uiGridExporterConstants', 'lookupTypeConfig', 'AdminJsonService', 'UiGridUtilService', 'ModalRowEdit', 'Dialog', '$rootScope', 'TransitionData', 'MessagesService', 'UiGridCommands', '$modal'];    
	
    function lookupAdminController($scope, $log, $filter, $interval, uiGridExporterConstants, lookupTypeConfig, AdminJsonService, UiGridUtilService, ModalRowEdit, Dialog, $rootScope, TransitionData, MessagesService, UiGridCommands, $modal) {
	var vm = this;
        ...
        function getLookupSearchResult(){
            MessagesService.clearMessages();
            vm.lookupTypePromise = AdminJsonService.getLookupType(vm.lookupType).then(function (data) {
                ...
                vm.gridOptions.data = $filter('orderBy')(vm.data, (vm.lookupType.sortId) ? vm.lookupType.sortId : UiGridUtilService.getFirstVisibleColumnId(colDefs), false);
...
//            }, 1, 1);
        ...
        function saveRow(rowEntity) {
            ...
            var promise = AdminJsonService.updateLookupType(vm.lookupType, rowEntity, vm.gridOptions.columnDefs);
...
//                        $log.debug(index);
        ...
        function isDuplicateFound(){
            ...
            if(dirtyRows.length > 0){
                var duplicates = UiGridUtilService.isDuplicateCheck(vm.gridOptions, dirtyRows, vm.lookupType.uniqueColumns);
        ...
        function isColumnInvalid(){
            var dirtyRows = vm.gridApi.rowEdit.getDirtyRows();
            if(dirtyRows.length > 0){
                var carrierInvalidIndices = UiGridUtilService.validateEmptyColumnValidationRule(vm.gridOptions, dirtyRows, vm.lookupType.rule);
        ...
        function save() {
            ...
            if (vm.isEmptyRowFound()) {
                ...
                MessagesService.addMessage("There are duplicate values found. Please replace these duplicate values in order to save your changes.", "error");
                //Temporary workaround - prevent user from using undo when duplicate is detected
                vm.undoStack.length = 0;
            }
        ...
        function changeLookupType(lookupTypeOldValue) {
            ...
            if(!!vm.undoStack && vm.undoStack.length > 0){
                ...
                $modal.open({
                    templateUrl: 'app/admin-app/lookupadmin/views/lookupAdminConfirmationModal.html',
                    scope: $scope,
                    size: 'lg'
...
//            }
            ...
            if (vm.selectedRow) {                    
                ...
                UiGridCommands.execute('RowAdd', vm.gridContext, {index: selectedIndex + 1, data:newRow});                    
            } 
            else {                    
