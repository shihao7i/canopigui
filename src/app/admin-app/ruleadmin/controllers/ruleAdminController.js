(function() {
    ...
    angular.module('admin.app').controller('RuleAdminController', ruleAdminController);

    ruleAdminController.$inject = ['$scope', '$log', '$filter', '$q', '$stateParams', '$templateCache', '$interval', 'uiGridExporterConstants',
                                   'uiGridConstants', 'ruleCategoryConfig', 'AdminJsonService', 'ModalRowEdit', 'UiGridUtilService', 'Dialog', '$rootScope',
                                   'TransitionData', 'MessagesService', 'UiGridCommands','$modal', '$timeout'];

    function ruleAdminController($scope, $log, $filter, $q, $stateParams, $templateCache, $interval, uiGridExporterConstants, uiGridConstants, ruleCategoryConfig, AdminJsonService, ModalRowEdit, UiGridUtilService, Dialog, $rootScope, TransitionData, MessagesService, UiGridCommands, $modal, $timeout) {
        ...
        function beginCellEdit(rowEntity) {
            ...
            UiGridCommands.execute('CellEdit', vm.gridContext, {index: index, rowEntity: rowEntity});
...
//            }, 1, 1);
        ...
        function rowSelectionChanged(row) {
            ...
            if(vm.selectedRow){
                if((vm.selectedRow.id) === undefined || vm.selectedRow.id === '' || (_.startsWith(vm.selectedRow.id, 'GUI')===true) ){
        ...
        function saveRow(rowEntity) {
            ...
            var promise = AdminJsonService.updateRuleCategory(vm.ruleCategory, rowEntity, vm.gridOptions.columnDefs);
        ...
        function getRuleCategorySearchResult(){
            ...
            vm.ruleCategoryPromise = AdminJsonService.getRuleCategory(vm.ruleCategory, vm.woType).then(function (data) {

                vm.data = UiGridUtilService.extractTableCellValues(data);

                var colDefs = UiGridUtilService.extractColumnDefs(data, null, vm.ruleCategory.pinnedColumns);
                //var colDefs = UiGridUtilService.extractColumnDefs(data);

                // sort the data on the first visible column in asc order
                vm.gridOptions.data = $filter('orderBy')(vm.data, (vm.ruleCategory.sortId) ? vm.ruleCategory.sortId : UiGridUtilService.getFirstVisibleColumnId(colDefs), false);
                
                //vm.gridOptions.data = vm.data;
                $log.debug(vm.gridOptions.data.length);
                vm.gridDataCopy = angular.copy(vm.gridOptions.data);

                colDefs = UiGridUtilService.autoColWidth(colDefs, data.rowMetadata, vm.ruleCategory.columnWidths);
        ...
        function search() {
            ...
            vm.gridOptions.gridMenuCustomItems = [{title: 'Hide Empty Columns',action: function () {vm.toggleEmptyColumns(false);}}]
        }
        ...
        function toggleEmptyColumns(isShowClicked){
            ...
            for (var i = 0; i < vm.gridOptions.columnDefs.length; i++) {
                if(_.includes(enabledColumns, vm.gridOptions.columnDefs[i].id) 
                    ...
                    if(isShowClicked) {
                        ...
                        vm.gridOptions.gridMenuCustomItems = [{title: 'Show Empty Columns',action: function () {vm.toggleEmptyColumns(true);}}];
                    }
                }
            }
        ...
        function insertCopyAtLocation(){
            ...
            UiGridCommands.execute('RowDuplicate', vm.gridContext, {index: selectedIndex, data:vm.dataCopy});
            //Set copied row to dirty
            $interval(function(){
                vm.gridApi.rowEdit.setRowsDirty([vm.gridOptions.data[vm.selectedIndex]] );
                //Also works
                //TransitionData.setDirtyRows('yes');
            }, 1, 1);
...
//            }, 1, 1);
        ...
        function changeRuleCategory(ruleCategoryOldValue) {
            ...
            if(!!vm.undoStack && vm.undoStack.length > 0){
                ...
                $modal.open({
                        templateUrl: 'app/admin-app/ruleadmin/views/ruleAdminConfirmationModal.html',
                        scope: $scope,
                        size: 'lg'
                        ...
                        if(vm.continueNextState) {
                            ...
                            if(vm.ruleCategory.title === "Task Queue" && vm.woTypes === undefined){
...
//                            });
        ...
        function addItem(item) {
            ...
            if(vm.selectedRow){
                vm.selectedIndex = vm.gridOptions.data.lastIndexOf(vm.selectedRow);
                UiGridCommands.execute('RowAdd', vm.gridContext, {index: vm.selectedIndex + 1, data:newRow});
                $log.debug(vm.gridOptions.data[vm.selectedIndex + 1]);
            }
            else{
                var lastIndex = vm.gridOptions.data.length;
                UiGridCommands.execute('RowAdd', vm.gridContext, {index: lastIndex, data:newRow});
            }
...
//            }, 1, 1);
        ...
        function discardItem(item) {
            ...
            var dataRows = selectedRows.map( function( selectedRow ) { return selectedRow.entity; });
...
//            }
        ...
        function isDuplicateFound(){
            ...
            if(dirtyRows.length > 0){
                var duplicates = UiGridUtilService.isDuplicateCheck(vm.gridOptions, dirtyRows, vm.ruleCategory.uniqueColumns);
        ...
        function isRuleInvalid() {
            ...
            var dirtyRows = vm.gridApi.rowEdit.getDirtyRows();
            if(dirtyRows.length > 0){
                var ruleInvalidIndices = UiGridUtilService.validateTaskDelegationRule(vm.gridOptions, dirtyRows, vm.ruleCategory.rule);
        ...
        function save() {
            ...
            $timeout(function() {
                if (vm.isEmptyRowFound()) {
                    ...
                    MessagesService.addMessage("Empty row(s) found in the table. Please either discard the empty row or enter values before saving again.", "error");
                } else if(!!vm.ruleCategory.rule && vm.isRuleInvalid().length > 0) {
                    MessagesService.addMessage(vm.ruleCategory.rule.errorMessage, "error");
                    ...
                    MessagesService.addMessage("There are duplicate values found at Row: " + duplicateIndices+ ". Please replace these duplicate values in order to save your changes.", "error");
                    //var message = ""
            ...
            for (var x = 0; x<gridRows.length; x++){
                ...
                for(var y = 0; y<vm.gridOptions.data.length; y++){
                    ...
                    if(vm.gridOptions.data[y].id === dirtyid){
                        ...
                        vm.dirtyIndices[x] = vm.gridOptions.data.lastIndexOf(rowTobeRemoved);
