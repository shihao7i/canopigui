(function() {
    ...
    mobilityOrdersController.$inject = ['$scope', '$templateCache', 'uiGridExporterConstants', '$log', '$q', '$interval', '$modal', 'orderPicklists', 'taskPriorityConfig', 'AdminJsonService', 'UiGridUtilService', 'MessagesService', 'Dialog', '$rootScope', 'TransitionData','UiGridCommands'];    
	
    function mobilityOrdersController($scope, $templateCache, uiGridExporterConstants, $log, $q, $interval, $modal, orderPicklists, taskPriorityConfig, AdminJsonService, UiGridUtilService, MessagesService, Dialog, $rootScope, TransitionData, UiGridCommands) {
	var vm = this;
        ...
        function populateDataForTaskListTable(tableData) {
            ...
            vm.gridOptionsTaskListTable.enableHorizontalScrollbar = UiGridUtilService.isHorizontalScrollbarNeeded(colDefs);
            vm.gridOptionsTaskListTable.exporterCsvFilename = vm.woType.title + '-tasks.csv';
        ...
        function populateDataForUDATable(tableData) {
            ...
            var colDefs = UiGridUtilService.extractColumnDefs(tableData);
            colDefs = UiGridUtilService.autoColWidth(colDefs, tableData.rowMetadata);
            vm.gridOptionsUDATable.enableHorizontalScrollbar = UiGridUtilService.isHorizontalScrollbarNeeded(colDefs);
        ...
        function populateDataForLinksTable(tableData) {
            ...
            var colDefs = UiGridUtilService.extractColumnDefs(tableData);
            colDefs = UiGridUtilService.autoColWidth(colDefs, tableData.rowMetadata);
            vm.gridOptionsLinksTable.enableHorizontalScrollbar = UiGridUtilService.isHorizontalScrollbarNeeded(colDefs);
        ...
        function buildColumnDefsAndDataForSelectedWO() {
            ...
            AdminJsonService.getWOSearchResults().then(function (data) {
                var tableData = data;
                vm.gridOptionsSelectedWO.data = UiGridUtilService.extractTableCellValues(tableData);
                vm.gridOptionsSelectedWO.columnDefs = UiGridUtilService.extractColumnDefs(tableData);
        ...
        function beginCellEditTaskList(rowEntity) {
            ...
            UiGridCommands.execute('CellEdit', vm.gridContextTaskList, {index: index, rowEntity: rowEntity});
        }
        ...
        function rowSelectionChangedTaskList(rowEntity) {
            ...
            $log.log(msg);            
            vm.selectedRowForTaskList = rowEntity.isSelected ? rowEntity.entity : false;
            if(vm.selectedRowForTaskList){                
                ...
                vm.selectedIndexForTaskList = vm.gridOptionsTaskListTable.data.lastIndexOf(vm.selectedRowForTaskList);
...
//                ]
        ...
        function moveSelectedUpForTaskList() {
            ...
            if (vm.previousIndexForTaskList > -1) {
                ...
                vm.previousRowDataForTaskList = vm.gridOptionsTaskListTable.data[vm.previousIndexForTaskList];

                vm.nextRowDataForTaskList = vm.gridOptionsTaskListTable.data[vm.nextIndexForTaskList];

                vm.gridOptionsTaskListTable.data[vm.previousIndexForTaskList] = vm.gridOptionsTaskListTable.data[vm.selectedIndexForTaskList];
                vm.gridOptionsTaskListTable.data[vm.selectedIndexForTaskList] = vm.previousRowDataForTaskList;
            ...
            vm.gridApiTaskList.core.queueRefresh().then(function () {
                vm.gridApiTaskList.selection.selectRow(vm.gridOptionsTaskListTable.data[vm.selectedIndexForTaskList]);
            });
        }
        ...
        function moveSelectedDownForTaskList() {
            ...
            if (vm.nextIndexForTaskList !== vm.gridOptionsTaskListTable.data.length) {                
                ...
                vm.previousRowDataForTaskList = vm.gridOptionsTaskListTable.data[vm.previousIndexForTaskList];
                vm.nextRowDataForTaskList = vm.gridOptionsTaskListTable.data[vm.nextIndexForTaskList];
                vm.gridOptionsTaskListTable.data[vm.nextIndexForTaskList] = vm.gridOptionsTaskListTable.data[vm.selectedIndexForTaskList];
                vm.gridOptionsTaskListTable.data[vm.selectedIndexForTaskList] = vm.nextRowDataForTaskList;
                vm.selectedIndexForTaskList = vm.selectedIndexForTaskList + 1;
                checkEndForTaskList();
                checkStartForTaskList();
            }
        ...
        function addTaskForTaskList() {
            ...
            $modal.open({
                ...
                size: 'lg',
                resolve: {
                    data: ['AdminJsonService', function (AdminJsonService) {
                ...
                .then(function (data) {
                    ...
                    if (!selectedRowExists) {
                        ...
                        if (vm.selectedRowForTaskList) {
                            ...
                            vm.selectedIndexForTaskList = vm.gridOptionsTaskListTable.data.lastIndexOf(vm.selectedRowForTaskList);
                            UiGridCommands.execute('RowAdd', vm.gridContextTaskList, {index: vm.selectedIndexForTaskList + 1, data: data});
                        } else {
                            var lastIndex = vm.gridOptionsTaskListTable.data.length;
                            UiGridCommands.execute('RowAdd', vm.gridContextTaskList, {index: lastIndex, data: data});
                        }
                    } else {
                        MessagesService.addMessage('Task Code [' + data.taskCode + '] is already added to list', "warning");
                    }
        ...
        function removeTaskForTaskList() {
            // Remove an item
            vm.gridOptionsTaskListTable.data.splice(vm.gridOptionsTaskListTable.data.lastIndexOf(vm.selectedRowForTaskList), 1);
            recalcTaskSequenceForTaskList();
        ...
        function beginCellEditUDA(rowEntity) {
            ...
            UiGridCommands.execute('CellEdit', vm.gridContextUDA, {index: index, rowEntity: rowEntity});
        }
        ...
        function rowSelectionChangedUDA(row) {
            ...
            if(vm.selectedRowForUDA.id === undefined || vm.selectedRowForUDA.id=== ''){
                vm.newRowForUDA = true;
            }
            else{
                vm.newRoworUDA = false;
            }
            vm.selectedIndexForUDA = vm.gridOptionsUDATable.data.lastIndexOf(vm.selectedRowForUDA);
...
//                ]
        ...
        function addUDA() {
            ...
            if (vm.selectedRowUDA) {
                var selectedIndex = vm.gridOptionsUDATable.data.lastIndexOf(vm.selectedRowUDA);
                UiGridCommands.execute('RowAdd', vm.gridContextUDA, {index: selectedIndex + 1, data: newRowForUDA});                
            }
            else {
                var lastIndex = vm.gridOptionsUDATable.data.length;
                UiGridCommands.execute('RowAdd', vm.gridContextUDA, {index: lastIndex, data: newRowForUDA});
            }
        }
        ...
        function discardUDA(){
            ...
            vm.gridOptionsUDATable.data.splice(vm.gridOptionsUDATable.data.lastIndexOf(vm.selectedRowUDA), 1);
            vm.newRowForUDA = false;               
        ...
        function beginCellEditLinks(rowEntity) {
            ...
            UiGridCommands.execute('CellEdit', vm.gridContextLinks, {index: index, rowEntity: rowEntity});
        }
        ...
        function rowSelectionChangedLinks(row) {
            ...
            if((vm.selectedRowForLinks.id) === undefined || vm.selectedRowForLinks.id ===''){
                vm.newRowForLinks = true;
            }
            else{
                vm.newRowForLinks = false;
            }
            vm.selectedIndexForLinks = vm.gridOptionsLinksTable.data.lastIndexOf(vm.selectedRowForLinks);
        }
...
//                ]
        ...
        function addLink() {                        
            ...
            if (vm.selectedRowForLinks) {
                var selectedIndex = vm.gridOptionsLinksTable.data.lastIndexOf(vm.selectedRowForLinks);
                UiGridCommands.execute('RowAdd', vm.gridContextLinks, {index: selectedIndex + 1, data: newRowForLinks});                
            }
            else {
                var lastIndex = vm.gridOptionsLinksTable.data.length;
                UiGridCommands.execute('RowAdd', vm.gridContextLinks, {index: lastIndex, data: newRowForLinks});                             
            }
        }    
            
        
        function discardLink() {   
            vm.gridOptionsLinksTable.data.splice(vm.gridOptionsLinksTable.data.lastIndexOf(vm.selectedRowForLinks), 1);
            vm.newRowForLinks = false;               
        ...
        function beginCellEdit(rowEntity) {
            ...
            UiGridCommands.execute('CellEdit', vm.gridContextNewWOTemplate, {index: index, rowEntity: rowEntity});
        ...
        function rowSelectionChangedNewWOTemplate(row) {
            ...
            if(vm.selectedRowForNewWOTemplate.id === undefined || vm.selectedRowForNewWOTemplate.id === ''){
                vm.newRowForNewWOTemplate = true;
            }
            else{
                vm.newRowForNewWOTemplate= false;
            }
            vm.selectedIndexForNewWOTemplate = vm.gridOptionsNewWOTemplateTable.data.lastIndexOf(vm.selectedRowForNewWOTemplate);
        ...
        function displayNewWOTemplateTable() {
            AdminJsonService.getWoTemplateColumnDefs().then(function (data) {
                ...
                colDefs = UiGridUtilService.autoColWidth(colDefs, tableData.rowMetadata);
                vm.gridOptionsNewWOTemplateTable.enableHorizontalScrollbar = UiGridUtilService.isHorizontalScrollbarNeeded(colDefs);
        ...
        function addNewWOTemplate() {
            ...
            if (vm.selectedRowForNewWOTemplate) {
                var selectedIndex = vm.gridOptionsNewWOTemplateTable.data.lastIndexOf(vm.selectedRowForNewWOTemplate);
                UiGridCommands.execute('RowAdd', vm.gridContextNewWOTemplate, {index: selectedIndex + 1, data: newRow});
            }
            else {
                var lastIndex = vm.gridOptionsNewWOTemplateTable.data.length;
                UiGridCommands.execute('RowAdd', vm.gridContextNewWOTemplate, {index: lastIndex, data: newRow});
            }
        }
        
        
        function removeNewWOTemplate() {
            vm.gridOptionsNewWOTemplateTable.data.splice(vm.gridOptionsNewWOTemplateTable.data.lastIndexOf(vm.selectedRowForNewWOTemplate), 1);
            vm.newRowForNewWOTemplate = false;
        ...
        function beginCellEditNewTaskList(rowEntity) {
            ...
            UiGridCommands.execute('CellEdit', vm.gridContextNewTaskList, {index: index, rowEntity: rowEntity});

        }

                
        function rowSelectionChangedNewTaskList(rowEntity) {
            vm.selectedRowForNewTaskList = rowEntity.isSelected ? rowEntity.entity : false;
            vm.selectedNewIndexForNewTaskList = vm.gridOptionsNewTaskListTable.data.lastIndexOf(vm.selectedRowForNewTaskList);
            ...
            //Is the selected row already the last Element? Then disable the down button
            //Is the selected row the first element in the list? Then disable the up button
            if (vm.nextNewIndexForNewTaskList === (vm.gridOptionsNewTaskListTable.data.length)) {
        ...
        function displayNewTaskListTable() {
            ...
            AdminJsonService.getTaskListColumnDefs().then(function (data) {
                ...
                colDefs = UiGridUtilService.autoColWidth(colDefs, tableData.rowMetadata);
                vm.gridOptionsNewTaskListTable.enableHorizontalScrollbar = UiGridUtilService.isHorizontalScrollbarNeeded(colDefs);
        ...
        function moveSelectedUpForNewTaskList() {
            ...
            if (vm.previousNewIndexForNewTaskList > -1) {
                ...
                vm.previousNewRowDataForNewTaskList = vm.gridOptionsNewTaskListTable.data[vm.previousNewIndexForNewTaskList];

                vm.nextNewRowDataForNewTaskList = vm.gridOptionsNewTaskListTable.data[vm.nextNewIndexForNewTaskList];

                vm.gridOptionsNewTaskListTable.data[vm.previousNewIndexForNewTaskList] = vm.gridOptionsNewTaskListTable.data[vm.selectedNewIndexForNewTaskList];
                vm.gridOptionsNewTaskListTable.data[vm.selectedNewIndexForNewTaskList] = vm.previousNewRowDataForNewTaskList;
                vm.selectedNewIndexForNewTaskList = vm.selectedNewIndexForNewTaskList - 1;
            ...
            vm.gridApiNewTaskList.core.refreshRows().then(function () {
                vm.gridApiNewTaskList.selection.selectRow(vm.gridOptionsNewTaskListTable.data[vm.selectedNewIndexForNewTaskList]);
            });
        }
        ...
        function moveSelectedDownForNewTaskList() {
            ...
            if (vm.nextNewIndexForNewTaskList !== vm.gridOptionsNewTaskListTable.data.length) {
                ...
                vm.gridOptionsNewTaskListTable.data[vm.nextNewIndexForNewTaskList] = vm.gridOptionsNewTaskListTable.data[vm.selectedNewIndexForNewTaskList];
                vm.gridOptionsNewTaskListTable.data[vm.selectedNewIndexForNewTaskList] = vm.nextNewRowDataForNewTaskList;
                vm.selectedNewIndexForNewTaskList = vm.selectedNewIndexForNewTaskList + 1;
                ...
                .then(function (data) {
                    ...
                    if (!selectedRowExists) {
                        ...
                        if (vm.selectedRowNewTaskList) {
                            //Pusing the selected Row after adidng the additional default values.
                            vm.selectedIndexForNewTaskList = vm.gridOptionsNewTaskListTable.data.lastIndexOf(vm.selectedRowForNewTaskList);
                            UiGridCommands.execute('RowAdd', vm.gridContextNewTaskList, {index: vm.selectedIndexForNewTaskList + 1, data: data});
                        } else {
                            var lastIndex = vm.gridOptionsNewTaskListTable.data.length;
                            UiGridCommands.execute('RowAdd', vm.gridContextNewTaskList, {index: lastIndex, data: data});
                        }
                    } else {
                        MessagesService.addMessage('Task Code [' + data.taskCode + '] is already added to list', "warning");
                    }
        ...
        function removeTaskForNewTaskList() {
            // Remove an item
            vm.gridOptionsNewTaskListTable.data.splice(vm.gridOptionsNewTaskListTable.data.lastIndexOf(vm.selectedRowForNewTaskList), 1);
            recalcTaskSequenceForNewTaskList();
        }
        ...
        function setupUiGridForNewAddTask() {
            ...
            return {
                ...
                onRegisterApi: function (gridApi) {
                    ...
                    // Register Events
                    gridApi.selection.on.rowSelectionChanged($scope, rowSelectionChangedNewAddTask);
                }
            };
         }
        ...
        function beginCellEditNewUDA(rowEntity) {
            ...
            UiGridCommands.execute('CellEdit', vm.gridContextNewUDA, {index: index, rowEntity: rowEntity});
        }
        ...
        function rowSelectionChangedNewUDA(row) {
            ...
            if(vm.selectedRowForNewUDA.id === undefined || vm.selectedRowForNewUDA.id === ''){
                vm.newRowForNewUDA = true;
            }
            else{
                vm.newRowForNewUDA = false;
            }
            vm.selectedIndexForNewUDA = vm.gridOptionsNewUDATable.data.lastIndexOf(vm.selectedRowForNewUDA);
        ...
        function displayNewUDATable() {
            ...
            AdminJsonService.getUDAColumnDefs().then(function (data) {
                ...
                colDefs = UiGridUtilService.autoColWidth(colDefs, tableData.rowMetadata);
                vm.gridOptionsNewUDATable.enableHorizontalScrollbar = UiGridUtilService.isHorizontalScrollbarNeeded(colDefs);
        ...
        function addNewUDA() {
            ...
            if (vm.selectedRowForNewUDA) {
                var selectedIndex = vm.gridOptionsNewUDATable.data.lastIndexOf(vm.selectedRowForNewUDA);
                UiGridCommands.execute('RowAdd', vm.gridContextNewUDA, {index: selectedIndex + 1, data: newRowForNewUDA});
            }
            else {
                var lastIndex = vm.gridOptionsNewUDATable.data.length;
                UiGridCommands.execute('RowAdd', vm.gridContextNewUDA, {index: lastIndex, data: newRowForNewUDA});                            
            }                                                                 
        }
        ...
        function discardNewUDA(){
            vm.gridOptionsNewUDATable.data.splice(vm.gridOptionsNewUDATable.data.lastIndexOf(vm.selectedRowForNewUDA), 1);
            vm.newRowForNewUDA = false;               
        ...
        function beginCellEditNewLinks(rowEntity) {
            ...
            UiGridCommands.execute('CellEdit', vm.gridContextNewLinks, {index: index, rowEntity: rowEntity});
        }
        ...
        function rowSelectionChangedNewLinks(row) {
            ...
            if((vm.selectedRowForNewLinks.id) === undefined || vm.selectedRowForNewLinks.id === ''){
                vm.newRowForNewLinks = true;
            }
            else{
                vm.newRowForNewLinks = false;
            }
            vm.selectedIndexForNewLinks = vm.gridOptionsNewLinksTable.data.lastIndexOf(vm.selectedRowForNewLinks);
        }
        ...
        function displayNewLinksTable() {
            ...
            AdminJsonService.getLinksColumnDefs().then(function (data) {
                ...
                colDefs = UiGridUtilService.autoColWidth(colDefs, tableData.rowMetadata);
                vm.gridOptionsNewLinksTable.enableHorizontalScrollbar = UiGridUtilService.isHorizontalScrollbarNeeded(colDefs);
        ...
        function addNewLink() {
            ...
            if (vm.selectedRowForNewLinks) {
                    var selectedIndex = vm.gridOptionsNewLinksTable.data.lastIndexOf(vm.selectedRowForNewLinks);
                    UiGridCommands.execute('RowAdd', vm.gridContextNewLinks, {index: selectedIndex + 1, data: newRowForNewLinks});
            }
            else {
                var lastIndex = vm.gridOptionsNewLinksTable.data.length;
                UiGridCommands.execute('RowAdd', vm.gridContextNewLinks, {index: lastIndex, data: newRowForNewLinks}); 
            }                  
        }
