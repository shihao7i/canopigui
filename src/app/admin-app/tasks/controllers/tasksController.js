(function() {
  ...
  angular.module('admin.app').controller('TasksController', tasksController);
  ...
  function tasksController(
      ...
      return {
        ...
        onRegisterApi: function(gridApi) {
          ...
          gridApi.selection.on.rowSelectionChanged(
            $scope,
            rowSelectionChangedAddTaskCodeAndName
          );
        }
      };
    }
    ...
    function beginCellEdit(rowEntity) {
      ...
      UiGridCommands.execute('CellEdit', vm.gridContextTasks, {
        index: index,
        rowEntity: rowEntity
      });
    }
    ...
    function beginCellEditSubTasks(rowEntity) {
      ...
      UiGridCommands.execute('CellEdit', vm.gridContextSubTasks, {
        index: index,
        rowEntity: rowEntity
      });
    }
    ...
    function rowSelectionChangedSubTask(row) {
      ...
      if (
        vm.selectedRowSubTask.id === undefined ||
        vm.selectedRowSubTask.id === ''
      ) {
        vm.newRowSubTask = true;
      } else {
        vm.newRowSubTask = false;
      }
      vm.selectedIndexSubTask = vm.gridOptionsSubTasks.data.lastIndexOf(
        vm.selectedRowSubTask
      );
    }
    ...
    function taskCodeAndNameLookup() {
      AdminJsonService.getTaskCodeNameList().then(function(data) {
        ...
        vm.taskCodeAndNameLookupTable.data = UiGridUtilService.extractTableCellValues(
          tableData
        );
        ...
        vm.taskCodeAndNameLookupTable.enableHorizontalScrollbar = UiGridUtilService.isHorizontalScrollbarNeeded(
          colDefs
        );
    ...
    function displaySubtasks() {
      ...
      AdminJsonService.getSubTasks().then(function(data) {
        ...
        var colDefs = UiGridUtilService.extractColumnDefs(data);
        colDefs = UiGridUtilService.autoColWidth(colDefs, data.rowMetadata);
        vm.gridOptionsSubTasks.enableHorizontalScrollbar = UiGridUtilService.isHorizontalScrollbarNeeded(
          colDefs
        );
        vm.gridOptionsSubTasks.columnDefs = colDefs;
        vm.gridOptionsSubTasks.exporterCsvFilename =
          vm.taskType.name + 'subtasks.csv';
      });
    }
    ...
    function addItemForTasks() {
      ...
      if (vm.selectedRow) {
        vm.selectedIndex = vm.gridOptions.data.lastIndexOf(vm.selectedRow);
        UiGridCommands.execute('RowAdd', vm.gridContextTasks, {
    ...
    function insertCopyAtLocationForTasks() {
      vm.selectedIndex = vm.gridOptions.data.lastIndexOf(vm.selectedRow);
      UiGridCommands.execute('RowDuplicate', vm.gridContextTasks, {
    ...
    function addItemForSubTasks() {
      ...
      if (vm.selectedRowSubTask) {
        vm.selectedIndex = vm.gridOptionsSubTasks.data.lastIndexOf(
          vm.selectedRowSubTask
        );
        UiGridCommands.execute('RowAdd', vm.gridContextSubTasks, {
          index: vm.selectedIndex + 1,
          data: newRow
        });
      } else {
    ...
    function discardItemForSubTasks() {
      vm.gridOptionsSubTasks.data.splice(
        vm.gridOptionsSubTasks.data.lastIndexOf(vm.selectedRowSubTask),
        1
      );
      vm.newRowSubTask = false;
    ...
    function exportFileForSubTasks() {
      vm.gridApiSubTasks.exporter.csvExport(
        vm.uiGridExporterConstants.ALL,
        vm.uiGridExporterConstants.ALL
      );
    }
