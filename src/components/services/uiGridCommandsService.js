  ...
  function uiGridCommandsService($log) {
    ...
    function rowAdd(options) {
      ...
      options.context.gridApi.core.queueRefresh().then(function() {
        options.context.gridApi.cellNav.scrollToFocus(
          options.context.gridOptions.data[options.index],
          options.context.gridOptions.columnDefs[0]
        );
        options.context.gridApi.selection.selectRow(
          options.context.gridOptions.data[options.index]
        );
      });
    }
    ...
    function rowDuplicate(options) {
      ...
      options.context.gridApi.core.queueRefresh().then(function() {
        options.context.gridApi.selection.selectRow(
          options.context.gridOptions.data[options.index + 1]
        );
      });
      //       $interval(function(){
