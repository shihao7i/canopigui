(function() {
  ...
  function httpInterceptorService($q, $log, MessagesService) {
    ...
    function requestError(rejection) {
      ...
      $log.info(
        '[request failure => headers ' +
          angular.toJson(rejection.config.headers)
      );
      $log.info(
        '[request failure => url ' + angular.toJson(rejection.config.url)
      );

      MessagesService.addMessage(
        'Request Error ' + rejection.status + ' at ' + rejection.config.url,
        'error'
      );
    ...
    function responseError(rejection) {
      ...
      $log.info(
        '[response failure => headers ' +
          angular.toJson(rejection.config.headers)
      );
      $log.info(
        '[response failure => url ' + angular.toJson(rejection.config.url)
      );

      MessagesService.addMessage(
        'Server Error ' + rejection.status + ' at ' + rejection.config.url,
        'error'
      );
