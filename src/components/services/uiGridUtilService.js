(function() {
  ...
  function uiGridUtilService(
    ...
    function makeEditable(column) {
      if (column.fieldType === 'input' || column.fieldType === 'dropdown') {
        var template = angular
          .injector(['canopi.templates'])
          .get('ui-grid/' + column.fieldType);
    ...
    function extractTableCellValues(tableData) {
      ...
      var totalRows = !tableData.rowValueList
        ? 0
        : tableData.rowValueList.length;
    ...
    function getEnabledColumns(tableData) {
      var enabledColumnNames = _.pluck(
        _.where(tableData.rowMetadata.columnList, { visible: true }),
        'id'
      );
    ...
    function isEmptyRowFound(gridOptions) {
      ...
      var columnsToOmit = _.pluck(
        _.where(columnDefs, { visible: false }),
        'id'
      );
    ...
    function isDuplicateCheck(gridOptions, unsavedData, uniqueColumns) {
      ...
      for (var row = 0; row < unsavedData.length; row++) {
        ...
        newRowDataWithoutIdField = _.omit(
          newRowDataWithoutIdField,
          '$$hashKey'
        );
        newRowDataWithoutIdField = removeDataNullValues(
          newRowDataWithoutIdField
        );
        newRowDataWithoutIdField = convertDataUppercase(
          newRowDataWithoutIdField
        );
        ...
        //Case 2: If a particular column needs to be unique
          ...
          newRowDataWithUniqueColumns = removeDataNullValues(
            newRowDataWithUniqueColumns
          );
          newRowDataWithUniqueColumns = convertDataUppercase(
            newRowDataWithUniqueColumns
          );
          $log.debug(
            'newRowData: ' + angular.toJson(newRowDataWithUniqueColumns)
          );
      ...
      for (var i = 0, totalColumns = columns.length; i < totalColumns; i++) {
        if (columns[i].fieldType === 'dropdown') {
          ...
          for (var j = 0, totalRows = rows.length; j < totalRows; j++) {
            ...
            var filteredArray = _.pluck(
              _.where(fieldOptions, { code: cellsInRow[i] }),
              'title'
            );
    ...
    // 'code' is what is being stored in COLM DB
    function convertCellValueFromTitleToCodeForDropdownType(
      rowEntity,
      columns
    ) {
      var fieldOptions = [];
      ...
      angular.forEach(columns, function(column) {
        ...
        if (column.fieldType === 'dropdown') {
          ...
          // an array of all elements that equivelent property values
          var filteredArray = _.pluck(
            _.where(fieldOptions, { title: rowEntity[columnIdValue] }),
            'code'
          );
          ...
          rowEntity[columnIdValue] =
            filteredArray.length > 0 ? filteredArray[0] : '';
        }
      });
    ...
    function validateEmptyColumnValidationRule(gridOptions, unsavedData, rule) {
      ...
      if (!!rule.invalidCondition.emptyColumns) {
        $log.debug(
          'matchColumns:' + angular.toJson(rule.invalidCondition.emptyColumns)
        );
      }
      ...
      for (var row = 0; row < unsavedData.length; row++) {
        ...
        var dirtyRow = unsavedData[row].entity;
        //$log.debug("dirtyrow:" + angular.toJson(dirtyRow));
        var rowDataColumnsToCheckForEmpty = _.pick(
          dirtyRow,
          rule.invalidCondition.emptyColumns
        );

        rowDataColumnsToCheckForEmpty = removeDataNullValues(
          rowDataColumnsToCheckForEmpty
        );
        $log.debug(
          'rowDataWithMatchColumns: ' +
            angular.toJson(rowDataColumnsToCheckForEmpty)
        );

        var dirtyColumnVal = true;
        var y;
        for (y in rowDataColumnsToCheckForEmpty) {
          if (!rowDataColumnsToCheckForEmpty[y]) dirtyColumnVal = false;
        }
    ...
    function validateTaskDelegationRule(gridOptions, unsavedData, rule) {
      ...
      if (!!rule.invalidCondition.matchColumns) {
        $log.debug(
          'matchColumns:' + angular.toJson(rule.invalidCondition.matchColumns)
        );
      }
      if (!!rule.invalidCondition.unmatchColumns) {
        $log.debug(
          'unmatchColumns:' +
            angular.toJson(rule.invalidCondition.unmatchColumns)
        );
      }
      ...
      for (var row = 0; row < unsavedData.length; row++) {
        ...
        var dirtyRow = unsavedData[row].entity;
        //$log.debug("dirtyrow:" + angular.toJson(dirtyRow));
        var rowDataWithMatchColumns = _.pick(
          dirtyRow,
          rule.invalidCondition.matchColumns
        );
        ...
        $log.debug(
          'rowDataWithMatchColumns: ' + angular.toJson(rowDataWithMatchColumns)
        );

        var rowDataWithUnmatchColumns = _.pick(
          dirtyRow,
          rule.invalidCondition.unmatchColumns
        );

        rowDataWithUnmatchColumns = removeDataNullValues(
          rowDataWithUnmatchColumns
        );
        rowDataWithUnmatchColumns = convertDataUppercase(
          rowDataWithUnmatchColumns
        );
        $log.debug(
          'rowDataWithUnmatchColumns: ' +
            angular.toJson(rowDataWithUnmatchColumns)
        );
        ...
        for (x in rowDataWithMatchColumns) {
          if (!rowDataWithMatchColumns[x]) dirtyMatchColumnVal = false;
        }

        var dirtyUnmatchColumnVal = true;
        var y;
        for (y in rowDataWithUnmatchColumns) {
          if (!rowDataWithUnmatchColumns[y]) dirtyUnmatchColumnVal = false;
        }

        if (dirtyMatchColumnVal && dirtyUnmatchColumnVal) {
          for (var i = 0; i < gridData.length; i++) {
            // select the unique columns for duplicate comparison
            currentRowDataToMatch = _.pick(
              gridData[i],
              rule.invalidCondition.matchColumns
            );
            ...
            if (
              angular.equals(currentRowDataToMatch, rowDataWithMatchColumns)
            ) {
              currentRowDataToUnmatch = _.pick(
                gridData[i],
                rule.invalidCondition.unmatchColumns
              );
              currentRowDataToUnmatch = removeDataNullValues(
                currentRowDataToUnmatch
              );
              currentRowDataToUnmatch = convertDataUppercase(
                currentRowDataToUnmatch
              );
              if (
                !angular.equals(
