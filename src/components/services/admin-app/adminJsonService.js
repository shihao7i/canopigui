(function() {
    ...
    angular.module('admin.service')
           .factory('AdminJsonService', adminJsonService);
	
    adminJsonService.$inject = ['$rootScope', '$q', '$http', '$interval', '$location', '$log', 'AdminGuiConstants', 'UiGridUtilService'];    
        
    function adminJsonService($rootScope, $q, $http, $interval, $location, $log, AdminGuiConstants, UiGridUtilService) {
        ...
        var contextRoot = (function() {  
            ...
            if (AdminGuiConstants.END_TO_END_REST_CALL) {
                if (AdminGuiConstants.MOCK_DATA_FLAG) {
                    ...
                    contextRoot = "http://" + $location.host() + ":" + $location.port() + "/admin/resources/";
                }
            }
            else {
                if (AdminGuiConstants.MOCK_DATA_FLAG) {
                    ...
                    contextRoot = 'http://' + AdminGuiConstants.HOSTNAME_PORT_SETTING_FOR_COLM_CALL_FROM_NETBEANS + '/admin/resources/';
                }
            }
...
//            }
        ...
        function getLookupTypeConfig() {
            ...
            $http.get(baseUrl + 'lookuptype-config.json').success(function (response) {
        ...
        */
        function getLookupType(lookupType, options) {
            ...
            var deferred = $q.defer();
            $http.get(baseUrl + lookupType.value + '.json').success(function (response) {
                var convertedData = UiGridUtilService.convertCellValueFromCodeToTitleForDropdownType(response);
        ...
        function getLookupTypeFromCOLM(lookupType, options) {
            ...
            if (AdminGuiConstants.END_TO_END_REST_CALL) {
                baseUrl = "http://" + $location.host() + ":" + $location.port() + "/admin/resources/" + lookupType.urlSuffix;
            }
            else {
                baseUrl = 'http://' + AdminGuiConstants.HOSTNAME_PORT_SETTING_FOR_COLM_CALL_FROM_NETBEANS + '/admin/resources/' + lookupType.urlSuffix;
            }
        ...
        function buildPostRequestObject(rowEntity, columnDefs) {
            ...
            var convertedRowEntity = UiGridUtilService.convertCellValueFromTitleToCodeForDropdownType(rowEntityWithoutHashKey, columnDefs);
        ...
        function getTaskPriorityConfig() {
            ...
            var deferred = $q.defer();
            $http.get(baseUrl + 'taskpriority-config.json').success(function (response) {
                deferred.resolve(response);
            }).error(function() {
                deferred.reject("http error");
            });
        ...
        function buildPostRequestObjectForBulkOperation(gridData, columnDefs, deletedIds) {
            ...
            for (var i=0; i < gridData.length; i++) {
                ...
                // convert title value to code value before sending postObject to COLM 
                convertedRowEntity = UiGridUtilService.convertCellValueFromTitleToCodeForDropdownType(rowEntityWithoutHashKey, columnDefs);
        ...
        function updateLookupType(lookupType, rowEntity, columnDefs) {
            ...
            if (!!lookupType.valuesToCopy) {
                UiGridUtilService.copyValuesToHiddenFields(rowEntity, lookupType.valuesToCopy);
            }
        ...
        function updateLookupTypeInCOLM(lookupType, postObject) {
            ...
            var deferred = $q.defer();
            $http.post(baseUrl, JSON.stringify(postObject)).success(function (response) {
        ...
        function getRuleCategoryConfig() {
            ...
            var deferred = $q.defer();
            $http.get(baseUrl + 'rulecategory-config.json').success(function (response) {
                deferred.resolve(response);
            }).error(function() {
                deferred.reject("http error");
            });
        ...
        function getRuleCategory(ruleCategory, options) {
            ...
            baseUrl = contextRoot + 'app/admin-app/mock/ruleadmin/response/' + ruleCategory.value + '.json'; 
        ...
        function getRuleCategoryFromCOLM(ruleCategory, options) {
            ...
            // hard-code for testing
            if (AdminGuiConstants.END_TO_END_REST_CALL) {
                baseUrl = "http://" + $location.host() + ":" + $location.port() + "/admin/resources/" + ruleCategory.urlSuffix + queryString;
            }
            else {
                baseUrl = 'http://' + AdminGuiConstants.HOSTNAME_PORT_SETTING_FOR_COLM_CALL_FROM_NETBEANS + '/admin/resources/' + ruleCategory.urlSuffix + queryString;
            }
        ...
        function updateRuleCategory(ruleCategory, rowEntity, columnDefs) {
            ...
            if (!!ruleCategory.valuesToCopy) {
                UiGridUtilService.copyValuesToHiddenFields(rowEntity, ruleCategory.valuesToCopy);
            }
        ...
        function updateRuleCategoryInCOLM(ruleCategory, postObject) {
            ...
            // hard-code for testing
            if (AdminGuiConstants.END_TO_END_REST_CALL) {
                baseUrl = "http://" + $location.host() + ":" + $location.port() + "/admin/resources/" + ruleCategory.urlSuffix;
            }
            else {
                baseUrl = 'http://' + AdminGuiConstants.HOSTNAME_PORT_SETTING_FOR_COLM_CALL_FROM_NETBEANS + '/admin/resources/' + ruleCategory.urlSuffix;
            }
        ...
        function getWOTypes() {
            ...
            // hard-code for testing
            if (AdminGuiConstants.END_TO_END_REST_CALL) {
                baseUrl = "http://" + $location.host() + ":" + $location.port() + "/admin/resources/choicelist/OrderType";
            }
            else {
                baseUrl = "http://" + AdminGuiConstants.HOSTNAME_PORT_SETTING_FOR_COLM_CALL_FROM_NETBEANS + "/admin/resources/choicelist/OrderType";
            }
        ...
        function updateTaskPriority(taskPriority, gridData, columnDefs, deletedIds) {
            
            // call COLM restful service for the operations which are available
            if (isCOMLCallForTaskPrioritySupported(taskPrority)) {
                var postObject = buildPostRequestObject(gridData, columnDefs, deletedIds);
        ...
        function updateTaskPriorityInCOLM(taskPriority, postObject) {
            ...
            // hard-code for testing
            if (AdminGuiConstants.END_TO_END_REST_CALL) {
                baseUrl = "http://" + $location.host() + ":" + $location.port() + "/admin/resources/" + taskPriority.urlSuffix;
            }
            else {
                baseUrl = 'http://' + AdminGuiConstants.HOSTNAME_PORT_SETTING_FOR_COLM_CALL_FROM_NETBEANS + '/admin/resources/' + taskPriority.urlSuffix;
            }
