/**
 * This directive is used to display multiselectdropdown menu.
 * 
 *   
   [Example of the directive usage in html]:
   
   <div ng-dropdown-multiselect="true" options="example1data" selected-model="example1model"></div>
 *  
 */
      ...
      if (scope.settings.closeOnBlur) {
        $document.on('click', function(e) {
          ...
          while (angular.isDefined(target) && target !== null && !parentFound) {
            if (
              _.contains(target.className.split(' '), 'multiselect-parent') &&
              !parentFound
            ) {
        ...
        if (
          ...
          if (scope.settings.smartButtonMaxItems > 0) {
            ...
            angular.forEach(scope.options, function(optionItem) {
              if (
                scope.isChecked(
                  scope.getPropertyForObject(optionItem, scope.settings.idProp)
                )
              ) {
                ...
                itemsText.push(
                  converterResponse ? converterResponse : displayText
                );
              }
            });
